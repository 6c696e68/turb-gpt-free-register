    ...
    for key in ("timestamp", "created_at", "createdAt", "date", "receivedAt", "time"):
        ...
        try:
            ...
            if "T" not in normalized and " " in normalized and "+" not in normalized[10:]:
                # Thời gian phân tách bằng khoảng trắng không múi giờ: xử lý theo UTC (cloudflare_temp_email hay gặp)
...
def _parse_raw_email(raw: str) -> dict[str, str]:
    ...
    try:
        ...
        # Dự phòng: ít nhất rút Subject/From từ header
...
def _standalone_otp_from_html(html: str) -> str | None:
    ...
    if not html:
        return None
    # Bỏ style, giảm nhiễu màu kiểu #353740
    ...
    for pattern in (
        ...
        # Lọc ngữ cảnh màu/số theo dõi rõ ràng làm ở tầng trên; ở đây lấy mã độc lập cuối gần OTP thân thư hơn
        if matches:
            return matches[-1]
    return None
...
def _otp_item(item: dict) -> dict:
    ...
    sender = (
        ...
        or item.get("sender")
        or item.get("source")  # trường danh sách cloudflare_temp_email
        or parsed.get("from")
        or ""
    )
    ...
    # Ưu tiên nhét mã 6 số độc lập vào đầu text, giúp extract_otp khớp OTP thân thư chứ không phải nhiễu header
...
def list_messages(jwt: str, *, limit: int = 20, offset: int = 0) -> list[dict]:
    ...
    # Khớp grokRegister-cpa: limit/offset là query bắt buộc
...
def fetch_latest_otp(
    ...
    while time.monotonic() < deadline:
        ...
        for item in messages:
            ...
            # Thiếu trường danh sách thì thử chi tiết
            ...
            if effective_ts > best_timestamp or (
                ...
                # Cùng một thư thì không đặt lại settle
                pass
