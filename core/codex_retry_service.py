...
def check_stop_requested(email: str) -> None:
    if is_stop_requested(email):
        raise CodexRetryStopped("Người dùng dừng thủ công 手动停止 Codex chạy bù")
...
def request_stop(email: str) -> dict:
    ...
    if not retrying:
        db.update_account_codex_status(email, "stopped", "Người dùng dừng thủ công 手动停止 (không thấy lần chạy bù đang chạy)")
        return {"ok": True, "message": "Không thấy lần chạy bù đang chạy, đã đánh dấu đã dừng", "state": "stopped", "running": False}

    injected = bool(thread_id and _async_raise(int(thread_id), CodexRetryStopped))
    db.update_account_codex_status(email, "stopped", "Người dùng dừng thủ công 手动停止 Codex chạy bù")
            ...
            with _RETRYING_LOCK:
                if key in _RETRYING and key in _STOP_REQUESTED:
                    try:
                        ...
                        status = ""
                    if status == "stopped":
                        logger.warning("[Codex chạy bù] trễ giải phóng chỗ giữ sau khi dừng: email=%s thread_id=%s", email, thread_id or "-")
                        _clear_state_locked(key)
    ...
    try:
        ...
        with p.open("a", encoding="utf-8") as f:
            f.write(f"{_dt.now().strftime('%H:%M:%S')} [WARNING] [Codex chạy bù] người dùng手动停止, đã gửi tín hiệu dừng injected={injected}\n")
    except Exception:
        logger.exception("Ghi Codex dừng log thất bại")
...
def run_worker(
    ...
    *,
    ...
    result: dict = {"status": "failed", "ok": False, "message": "Chạy bù Codex không trả về kết quả"}
    key = (email or "").strip().lower()
    try:
        with _RETRYING_LOCK:
        ...
        try:
            ...
            logger.info(
                "[Codex chạy bù] đã nạp nóng cấu hình: CODEX_OAUTH_DRIVER=%s ROXY_OPEN_HEADLESS=%s ROXY_KEEP_BROWSER_OPEN=%s",
                getattr(codex_cfg, "CODEX_OAUTH_DRIVER", ""),
                getattr(roxy_cfg, "ROXY_OPEN_HEADLESS", ""),
                getattr(roxy_cfg, "ROXY_KEEP_BROWSER_OPEN", ""),
            )
        except Exception as exc:
            logger.warning("[Codex chạy bù] nạp nóng cấu hình thất bại, sẽ tiếp tục dùng cấu hình trong bộ nhớ hiện tại: %s: %s", type(exc).__name__, exc)

        if batch_label:
            logger.info("[Codex chạy bù] tác vụ hàng loạt: %s", batch_label)
        logger.info("[Codex chạy bù] Bắt đầu: %s", email)
        logger.info("[Codex chạy bù] mô tả giai đoạn: lấy URL uỷ quyền → đăng nhập email → email OTP → xác minh điện thoại → bắt callback → gửi/lưu thông tin xác thực")
        check_stop_requested(email)
        ...
        if result.get("ok"):
            ...
            logger.warning("[Codex chạy bù] %s Tài khoản đã chết: %s", email, result.get("message"))
        else:
        ...
        result = {"status": "stopped", "ok": False, "message": str(exc) or "Người dùng dừng thủ công 手动停止 Codex chạy bù"}
        db.update_account_codex_status(email, "stopped", result["message"])
        logger.warning("[Codex chạy bù] %s đã dừng: %s", email, result["message"])
        return result
    except Exception as exc:
        if is_stop_requested(email):
            result = {"status": "stopped", "ok": False, "message": "Người dùng dừng thủ công 手动停止 Codex chạy bù"}
            db.update_account_codex_status(email, "stopped", result["message"])
        ...
        logger.exception("[Codex chạy bù] %s bất thường", email)
        logger.error("[Codex chạy bù] đã kết thúc: thất bại bất thường")
        return result
    finally:
        try:
