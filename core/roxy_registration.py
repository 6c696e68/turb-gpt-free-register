...
def _wait_for_email_input(driver, timeout: int | None = None):
    ...
    for refresh_retry in range(_MISSING_PAGE_ELEMENT_REFRESH_RETRIES + 1):
        ...
        if not _refresh_after_missing_page_element(driver, "Ô nhập email/lối vào email", refresh_retry):
            break
    raise RuntimeError(
        f"không tìm thấy ô nhập email/lối vào email, đã làm mới và thử lại{_MISSING_PAGE_ELEMENT_REFRESH_RETRIES}次"
    ...
    for attempt in range(1, attempts + 1):
        ...
        if advanced:
            if advanced == "login_password":
                raise RuntimeError(f"邮箱提交后进入登录密码页, Coi như đã đăng ký/Xử lý email không dùng được và vô hiệu hoá: url={getattr(driver, 'current_url', '') or 'https://auth.openai.com/log-in/password'}")
        ...
        try:
            ...
            if exc.state == "login_password":
                raise RuntimeError(f"邮箱提交后进入登录密码页, Coi như đã đăng ký/Xử lý email không dùng được và vô hiệu hoá: url={getattr(driver, 'current_url', '') or 'https://auth.openai.com/log-in/password'}") from exc
...
def _fetch_chatgpt_session(driver, timeout: int = 90, auto_jump_wait: int = 15) -> dict:
    ...
    raise RuntimeError(f"等待 /api/auth/session accessToken 超时, phản hồi cuối: {str(last_data)[:800]}")
...
def run_roxy_registration(
    ...
    def _merge_proxy_transport_traffic() -> None:
        ...
        if browser_total > 0 and transport_total > browser_total * 1.2:
            logger.warning(
                "[Roxy] CDP Thống kê trang hiện tại %.2f MiB, proxy chain toàn trình duyệt thực tế %.2f MiB (%.2fx); "
    ...
    try:
        ...
        if _is_signup_password_page(driver):
            raise RuntimeError(
                f"Sau khi xử lý trang mật khẩu xong vẫn ở lại trang mật khẩu đăng ký: "
