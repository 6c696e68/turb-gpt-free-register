# -*- coding: utf-8 -*-
"gói/Plus tư cách tra cứu sau nền hàng đợi. "
...
def _wait_for_rate_slot() -> None:
    "là chỗ có tra cứu luồng cấp lệch request khởi động khi khoảng. "
    ...
    *,
    ...
    try:
        if not db.mark_account_plan_check_running(account_id):
            return {"ok": False, "error": "Tài khoản đã xoá hoặc trạng thái tra gói đã bị đặt lại"}
        ...
        if should_recheck:
            logger.info("[Plan] tài khoản mới tạm chưa thấy Plus dùng thử tư cách, %.1fs rồi kiểm tra lại một lần: %s", recheck_delay, email)
            ...
            if recheck_result.get("ok"):
                ...
                logger.warning(
                    "[Plan] kiểm tra lại tư cách tài khoản mới thất bại, giữ kết quả thành công lần đầu: %s, %s",
                    email,
                    recheck_result.get("error") or "Lỗi không xác định",
                )

        db.update_account_plan_check(acc_id=account_id, result=result)
        if result.get("ok"):
            logger.info(
                "[Plan] tra cứu nền thành công: %s, plan=%s, plus_trial=%s, trigger=%s",
                email,
            ...
            logger.warning(
                "[Plan] tra cứu nền thất bại: %s, trigger=%s, error=%s",
                email,
                trigger,
                result.get("error") or "Lỗi không xác định",
            )
        return result
        ...
        try:
            ...
            logger.exception("[Plan] ghi trạng thái ngoại lệ tra cứu nền thất bại: account_id=%s", account_id)
        logger.exception("[Plan] tra cứu nền ngoại lệ: %s", email)
        return result
    finally:
...
def enqueue_account_plan_check(
    *,
    ...
    " đưa tra cứu vào chung luồng kho; tra cứu trùng hoặc hàng đợi đầy khi không gửi. "
    ...
    if not _QUEUE_SLOTS.acquire(blocking=False):
        return {"accepted": False, "busy": False, "queue_full": True, "error": "Hàng đợi tra gói đã đầy, vui lòng thử lại sau"}

    if not db.claim_account_plan_check(acc_id=account_id, trigger=trigger):
        _QUEUE_SLOTS.release()
        return {"accepted": False, "busy": True, "error": "tài khoản này đang tra gói"}
    ...
    try:
        ...
        result = {
            "ok": False,
            "checked_at": datetime.now().isoformat(timespec="seconds"),
            "error": f"xếp hàng tra gói thất bại: {type(exc).__name__}: {str(exc)[:160]}",
        }
        db.update_account_plan_check(acc_id=account_id, result=result)
