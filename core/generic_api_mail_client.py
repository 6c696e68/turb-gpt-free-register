...
def _public_inbox_latest_code_url(code_url: str) -> str | None:
    ...
    if not match:
        # Cũng chấp nhận người dùng nhập thẳng địa chỉ API latest-code.
...
def _fetch_public_inbox_page_otp(
    ...
    for item in items:
        ...
        # Preview danh sách trang vẫn chưa rút được thì đọc API chi tiết mà trang dùng khi bấm thư.
...
def _extract_code(text: str) -> str | None:
    ...
    # Tương thích JSON: ưu tiên làm phẳng mọi value rồi rút.
...
def _extract_yangyang_openai_code(subject: str, body: str) -> str | None:
    ...
    # Bỏ style/script, giảm nhiễu số 6 chữ số của màu CSS, chiều rộng, v.v.
    ...
    # Lọc nhiễu template đã biết; giữ các ứng viên 6 số còn lại.
    ...
    # Thư mã tạm OpenAI: sau khi dọn nhiễu, số nghiệp vụ 6 chữ số cuối ổn định nhất.
...
def _extract_structured_api_code(text: str, after_ts: float | None = None) -> tuple[str, dict] | None:
    ...
    # Ưu tiên trường thường gặp: code / otp / verification_code; không có thì lùi rút từ văn bản đã làm phẳng.
    raw_code = (
        data.get("code")
        or data.get("otp")
...
def _fetch_yangyang_otp(
    ...
    # Thường trang đầu đủ; thận trọng hỗ trợ tối đa 5 trang.
    for _ in range(5):
        ...
        if resp.status_code != 200:
            if resp.status_code == 404:
                # Tương thích kiểu mail.ai1998.xyz cũng là /messages/{token}/{email},
                # nhưng không có /api/messages, thư nhúng thẳng trong trang HTML.
    ...
    # API mặc định thư mới ở trước; sắp lại thời gian giảm dần, cố lấy mã OTP mới nhất.
...
def _fetch_inline_messages_page_otp(
    *,
    ...
    # Không có article thì lùi chia theo details, tránh class đổi nhẹ.
    if not cards:
...
def fetch_latest_otp(
    ...
    # Tương thích cấu hình cũ: chưa đặt proxy riêng API chung thì dùng kho proxy; proxy riêng ưu tiên.
    ...
    while time.time() < deadline:
        attempt += 1
        try:
            # Không sửa URL dạng path của yangyang; API danh sách của nó tự trả dữ liệu theo ID thư.
            ...
            if resp is None:
                ...
                if mailbox_mismatch:
                    ...
                    # receivedAt của dịch vụ latest-code công khai này có thể dùng giờ server riêng,
                    # lệch máy đang chạy vài giờ thậm chí sang ngày. Nó chỉ trả "thư mới nhất", nên ở đây
                    # trường thời gian chỉ để chẩn đoán, không lọc cứng; cập nhật ứng viên vẫn do
                ...
                if code:
                    if (
                        ...
                        logger.warning(
                            "[GenericAPI] receivedAt của latest-code sớm hơn mốc lấy mã,"
                            "vẫn lấy thư mới nhất phía server làm ứng viên: receivedAt=%s after=%s messageId=%s",
